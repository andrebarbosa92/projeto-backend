...
public class MoradiaService {
	...
	public ServiceResponse inserirMoradia(Moradia moradia) {
		...
		if (locador.get().getTipoUsuario() != TipoUsuario.toEnum(2).getCodigo())
			throw new RegisterException("ID Locador inválido");
	...
	@Transactional(readOnly = true)
	public Page<AnuncioDTO> listarMoradiaPorLocador(Integer page, Integer linesPerPage, String orderBy,
			String direction) {
		...
		if (locador.get().getTipoUsuario() != TipoUsuario.LOCADOR.getCodigo())
			throw new ObjectNotFoundException("Nenhum anuncio encontrado!.");
		Page<Moradia> list = moradiaRepository.findByLocador(locador.get(), pageRequest);
		Page<AnuncioDTO> dto = list.map(u -> new AnuncioDTO(u));
		return dto;
	}

	public Page<AnuncioDTO> findByTipoMoradiaAndPreco(String tipo, Double preco, Integer page, Integer linesPerPage,
